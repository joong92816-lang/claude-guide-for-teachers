...
npm run monitor  # (선택) 소식 모니터링 → content/updates 초안 (GEMINI_API_KEY 필요)
```
...
- **모니터(선택)**: `scripts/monitor-updates.mts` 도 Gemini(`GEMINI_MODEL`) + Google 검색
  그라운딩을 쓴다. (그라운딩과 `responseSchema` 는 동시 사용 불가 → 프롬프트로 JSON 을
  요청하고 관대하게 파싱.) `GEMINI_API_KEY` 가 없으면 이 스크립트/워크플로만 실패하고
  사이트에는 영향 없다. 안 쓰면 `.github/workflows/monitor.yml` 을 비활성화하면 된다.
